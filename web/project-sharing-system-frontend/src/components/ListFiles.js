...
const navigate = useNavigate();
  ...
  const handleClick = () => {
    ...
    // Second API call
    fetch(`http://localhost:8080/deleteProjectHandler?projectID=${location.state.projID}&UserID=${user.name}&project_title=${location.state.projTitle}`, {
      method: 'GET',
    })
      .then(response => {
        if (!response.ok) {
  ...
  const handleButton = () => {
    ...
    fetch(`http://localhost:8080/downloadZip?UserID=${user.name}&projectID=${location.state.projID}`)
      .then(response => {
